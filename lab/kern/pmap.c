...
mem_init(void)
{
	...
	// LAB 3: Your code here.
	if (pse_support == 1) {
		boot_kernel_map(kern_pgdir, UENVS, PTSIZE, PADDR(envs), PTE_U);
	} else {
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (!(pgdir[pdx] & PTE_P)) {
		...
		new_page->pp_ref = 1;
		pgdir[pdx] = page2pa(new_page) | PTE_P | PTE_U | PTE_W;
	}	
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	pa = page2pa(pp);
	if (*pte & PTE_P) {
		/* If the entry is present and pp physical address is the same,
		 * update permission flags and return successfully.
		 */
		if (PTE_ADDR(*pte) == pa) {
			*pte = pa | perm | PTE_P;
			return 0;
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	if (base + size_rounded >= MMIOLIM || base + size_rounded < base) {
